/* W3.JS 1.05 December 2023 by w3schools.com */
"use strict";
const w3 = {};
...
w3.show = function (sel, a) {
    const elements = w3.getElements(sel);
    if (a) {
...
w3.toggleClassElement = function (element, c1, c2) {
    let t1, t2, t1Arr, t2Arr, allPresent;
    t1 = c1 || "";
    t2 = c2 || "";
    t1Arr = t1.split(" ");
    t2Arr = t2.split(" ");
...
w3.filterHTML = function (id, sel, filter) {
    let c;
    const a = w3.getElements(id);
    for (let i = 0; i < a.length; i++) {
        let b = a[i].querySelectorAll(sel);
        for (let ii = 0; ii < b.length; ii++) {
            let hit = 0;
            if (
                b[ii].innerText.toUpperCase().indexOf(filter.toUpperCase()) > -1
            ) {
                hit = 1;
            }
...
(w3.dataObject = {}),
    (w3.displayObject = function (id, data) {
        let a, rowClone, x, i, cc, repeat, repeatObj;
        const htmlObj = document.getElementById(id);
        const htmlTemplate = init_template(id, htmlObj);
        let html = htmlTemplate.cloneNode(true);
        const arr = w3.getElementsByAttribute(html, "w3-repeat");
        let repeatX = "";
        let l = arr.length;
        for (let j = l - 1; j >= 0; j -= 1) {
        ...
        function w3_replace_curly(elmnt, typ, repeatX, x) {
            let value, rowClone, pos2, originalHTML, lookFor, cc;
            let lookForARR = [];
            ...
            while (pos1 > -1) {
                ...
                value = undefined;
                for (let i = 0; i < lookForARR.length; i += 1) {
